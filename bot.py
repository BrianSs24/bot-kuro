...
BOTS_PERMITIDOS = ["MineLatino", "Ultimate Clans V7"]
...
# La semana termina el domingo a las 23:59:59 hora de RD.
DIA_FIN_SEMANA = 6
HORA_FIN_SEMANA = 23
...
async def comprobar_cierre_semanal():
    ...
    for canal_id in (CANAL_KURO_ID, CANAL_TNA_ID):
        ...
        try:
            ...
            await enviar_bloque(
                canal,
                "♻️ Las puntuaciones semanales serán reiniciadas ahora."
            )
        except Exception as e:
            print("❌ ERROR ENVIANDO TOP SEMANAL FINAL:", e)

    # IMPORTANTE: solo se borran las tablas SEMANALES.
    # puntos_kuro y puntos_tna NO se tocan.
...
async def revisar_semana():
    try:
        ...
        print("❌ ERROR REVISANDO SEMANA:", repr(e))
...
async def on_ready():
    ...
    print("========================================")
    print("📋 COMANDOS SEMANALES CARGADOS:")
    print("!topkurosemanal")
    print("!toptnasemanal")
    print("!puntoskurosemanal <usuario>")
    print("!puntostnasemanal <usuario>")
    print("!resetsemanal")
    print("!activarsemanal")
    print("!desactivarsemanal")
    print("!tablasemanalfinal kuro|tna|ambas")
    print("!configsemanal")
    print("!semanainfo")
    print("========================================")
...
def extraer_datos(texto):
    ...
    #
    # Acepta nombres normales y nombres que empiezan por punto:
    # .josuejefe15
    ...
    if match_nuevo:
        ...
        puntos = int(
            match_nuevo.group(2).replace(".", "").replace(",", "")
        )
...
async def topkuro(ctx):
    ...
    data = ejecutar("""
        SELECT usuario, puntos FROM puntos_kuro
        ORDER BY puntos DESC
    """, fetch=True)
...
async def toptna(ctx):
    ...
    data = ejecutar("""
        SELECT usuario, puntos FROM puntos_tna
        ORDER BY puntos DESC
    """, fetch=True)
...
async def totalkuro(ctx):
    ...
    data = ejecutar("SELECT SUM(puntos) FROM puntos_kuro", fetch=True)
...
async def totaltna(ctx):
    ...
    data = ejecutar("SELECT SUM(puntos) FROM puntos_tna", fetch=True)
    total_bd = data[0][0] or 0
...
@bot.command(name="topkurosemanal")
...
@bot.command(name="toptnasemanal")
...
@bot.command(name="puntoskurosemanal")
...
@bot.command(name="puntostnasemanal")
...
async def resetsemanal(ctx):
    try:
        ...
        print("❌ ERROR RESET SEMANAL:", repr(e))


@bot.command(name="activarsemanal")
@commands.has_permissions(administrator=True)
...
@bot.command(name="desactivarsemanal")
@commands.has_permissions(administrator=True)
...
@bot.command(name="tablasemanalfinal")
@commands.has_permissions(administrator=True)
async def tablasemanalfinal(ctx, opcion: str):
    ...
    if opcion not in ("kuro", "tna", "ambas"):
        await ctx.send(
            "❌ Opción inválida.\n"
            "`!tablasemanalfinal kuro`\n"
            "`!tablasemanalfinal tna`\n"
            "`!tablasemanalfinal ambas`"
        )
        return
    ...
    await ctx.send(
        f"✅ Tabla(s) al finalizar: **{opcion.upper()}**"
    )


@bot.command(name="configsemanal")
...
async def on_command_error(ctx, error):
    if isinstance(error, commands.CommandNotFound):
        # No mostrar spam por comandos desconocidos.
        print(f"⚠️ Comando desconocido recibido: {ctx.message.content}")
        return
